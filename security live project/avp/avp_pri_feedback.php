...
if (isset($_POST['sub']))
{
  ...
  if ($feed=="Approved")
	...
	{
			echo "<script> alert('Please Provide Comments!');location.href='avp_pri_feedback.php?id=$ticket_no';</script>";
	}
	else
	{
	
	$web = "update `pri_sim_request` set avp_feedback='$feed', avp_comments= '$reason', avp_status= '$avp_status' where ticket_no='$ticket_no'";
		...
		if($wires == true)
		{
			...
			$trow=mysqli_fetch_array($wires2);
				...
				if ($trow==true)
				{
						...
						if($trow['avp_status']=='1')
							{
							 
							 $process_name=$trow['process_name'];

							
							$mail->addAddress('pinaki.narendra@silaris.in','CIO');					
							
							$mail->Subject="PRI/SIM Card Request";
							$mail->Body="Dear Sir, <br><br>

            PRI/SIM Card request for <strong>  " .$process_name. "</strong>  has been <strong> Approved </strong> by IT-AVP . Kindly login at http://isms.silaris.in  to see details. <br> <br> Thank You! <br> <br><br><strong> Note: This is an auto generated mail.</strong>";

							$mail->send();

							echo "<script> alert('Information Saved Successfully!');location.href='pri_sim_request.php';</script>";
						}
		...
		else
		{
			...
			header('location:avp_pri_feedback.php');
