...
if(isset($_POST['comsub']))
{
	...
	$esql = "UPDATE `sc_emailcreation` SET `sc_suggestemail`='$emailnt',`sc_itdepart`='1', `sc_itdpartcomment`='$comment',`sc_action`='1' WHERE `sc_sno`='$sn'";
	$eres = mysqli_query($conn,$esql);
	if($eres == true)
	{

		$mail->addAddress($ueml,$unam);
		$mail->addCC('email.support@silaris.in', 'Arun Dogra');
		$mail->Subject = 'Email ID Creation Request';
    	$mail->Body.= 'Hi, <br>Kindly login in http://isms.silaris.in.<br><br>';
			...
			<?php
            		if(isset($_GET['sid']))
                    {
                          ...
                          <div class="form-group">
                          	<label>Creating Email Address</label>
                          	<input type="text" name="emailnt" class="form-control" require>
