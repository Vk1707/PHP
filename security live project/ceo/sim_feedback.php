...
if (isset($_POST['sub']))
{
  ...
  if ($feed=="Approved")
	{
		...
		for($i=1;$i<=$pc;$i++)
		{
				$sql2 = "INSERT INTO `sim_tracker`(`sr_no`, `provided_by`,`received_on`,`request_date`,`mobile_number`, `sim_number`, `sim_type`, `process`, `avp_email`,`gateway_ip`, `port`, `activation_date`, `allocation_date`, `updated_thrg`) VALUES ('$sr_no', 'Provided by Silaris','','$rd','Not Updated','$sim','$type','$pr','$avp_email','Not Updated','Not Updated','','','Manually')";
 ...
 if (($feed=="Rejected" AND $reason=="")OR ($feed=="On Hold" AND $reason==""))
	{
			echo "<script> alert('Please Provide Comments!');location.href='sim_feedback.php?id=$ticket_no&p=$pc&t=$type&r=$rd&pr=$pr&e=$avp_email';</script>";
	}
	else
	{
